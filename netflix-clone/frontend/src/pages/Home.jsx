...
const Home = () => {
  ...
  }, [filters, retryCount, initialized]);
  ...
  const fetchInitialData = async () => {
    try {
      ...
      // Mark initialized so the videos effect can run once
      setInitialized(true);
    } catch (err) {
  ...
  const fetchVideos = async () => {
    try {
      if (!initialLoading) setListLoading(true);
      setError(null);
      ...
      <div style={containerStyle}>
        ...
        {isSearching ? (
          ...
          <div>
            ...
            {categories.length === 0 && !initialLoading && !listLoading && (
              <>
                {videos.length === 0 ? (
                  <NoVideosFound onRefresh={handleRetry} />
                ) : (
