...
try {
  ...
  console.warn('Stripe is not configured. Payments will be disabled.');
  stripe = null;
}
...
const createCheckoutSession = async (req, res) => {
  try {
    ...
    if (!stripe) {
      return res.status(503).json({ message: 'Payments are not configured', code: 'PAYMENTS_DISABLED' });
    }

    // Create checkout session (subscription mode does not allow payment_intent_data)
...
const getPaymentHistory = async (req, res) => {
  try {
    ...
    const [payments] = await db.execute(historyQuery, [req.user.id]);
